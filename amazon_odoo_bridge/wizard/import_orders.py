...
class ImportMwsOrders(models.TransientModel):
    ...
    def get_order_line_feeds(self,order_id,order_data):
        ...
        if order_items:
            ...
            if size==1:
                asin,order_item = list(order_items.items())[0]
    ...
    def import_amazon_orders(self,channel_id,create_ids=[],update_ids=[], nextToken=None):
        ...
        if not orders:
            ...
            self._cr.commit()
            if nextToken:
                self.import_amazon_orders(channel_id,create_ids=create_ids,update_ids=update_ids,nextToken=nextToken)
        return dict(
    ...
    def _cron_mws_import_order(self):
        for channel_id in self.env['multi.channel.sale'].search(CHANNELDOMAIN):
            ...
            try:
                ...
                _logger.info("Import Order Failed for channel=====(%r)====(%r)",channel_id,e)
                continue
        ...
        for channel_id in self.env['multi.channel.sale'].search(CHANNELDOMAIN):
            ...
            try:
                ...
                if report_id:
                    ...
                    import_res = obj.import_amazon_orders_status(channel_id,nextToken=False)
                if channel_id.debug=='enable':
                    _logger.info("message %r=import_res==%r=="%(message,import_res))
            except Exception as e:
                _logger.info("Import Order Status Failed =====(%r)====(%r)",channel_id,e)
                continue
